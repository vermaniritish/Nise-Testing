<div class="dropdown filter-dropdown">
	...
	<div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
		<form action="<?php echo route('partnerAdmin.testManagements') ?>" id="filters-form">
			<a href="javascript:;" class="float-right px-2 closeit"><i class="fa fa-times-circle"></i></a>
			<div class="dropdown-item">
				<div class="row">
					<div class="col-md-12">
						<label class="form-control-label">Select Assigned Lab:</label>
						<select class="form-control" name="test_service[]" multiple>
					      	<?php foreach($testingServices as $tesSer): ?>
					      		<option value="<?php echo $tesSer->id ?>" <?php echo isset($_GET['test_service']) && in_array($tesSer->id, $_GET['test_service'])  ? 'selected' : '' ?>><?php echo $tesSer->title ?></option>
					  		<?php endforeach; ?>
					    </select>
					</div>
				</div>
			...
			<div class="dropdown-divider"></div>
			<a href="<?php echo route('partnerAdmin.testManagements') ?>" class="btn btn-sm py-2 px-3 float-left">
				Reset All
			</a>
