<div class="dropdown filter-dropdown">
	...
	<div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
		<form action="<?php echo route('admin.contactUs') ?>" id="filters-form">
			<a href="javascript:;" class="float-right px-2 closeit"><i class="fa fa-times-circle"></i></a>
			<div class="dropdown-item">
				<div class="row">
					<div class="col-md-12">
						<label class="form-control-label">Contact Us</label>
						{{-- <select class="form-control" name="category[]" multiple>
							@foreach($categories as $category)
								<option value="{{ $category }}" {{ isset($_GET['category']) && in_array($category, $_GET['category']) ? 'selected' : '' }}>
									{{ ucwords($category) }}
								</option>
							@endforeach
						</select>						 --}}
			...
			<div class="dropdown-divider"></div>
			<a href="<?php echo route('admin.contactUs') ?>" class="btn btn-sm py-2 px-3 float-left">
				Reset All
			</a>
