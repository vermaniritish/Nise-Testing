...
@endif
			...
			<div class="row">
				<div class="col-lg-8 col-md-8 col-sm-12 col-12 mb-lg-0 mb-md-4 mb-sm-4 mb-4">
					<div class="calltoaction-two-wrap">
						<h4>
							<span>{{ $lang == 'hi' ? $yellowTitleHi : $yellowTitleEn }}</span>
						</h4>
						<p style="color:#fff;">
							<span>{{ $lang == 'hi' ? $yellowHeadingHi : $yellowHeadingEn }}</span>
						</p>
					</div>
				</div>
				<!-- end col -->
				@if(CustomPageData::get('yellow_button' . ($lang == 'hi' ? '_hi' : '')) == 1)
				<div class="col-lg-4 col-md-4 col-sm-12 col-12 mt-lg-4 mt-md-4 text-lg-right text-md-right text-sm-left text-left">
					<a href="<?php echo CustomPageData::get('yellow_button_link' . ($lang == 'hi' ? '_hi' : '')) ?>" class="btn-style btn-border btn-border-3">
						<span>{{ $lang == 'hi' ? $yellowButtonTextHi : $yellowButtonTextEn }}</span>
					</a>
				</div>
				@endif
				<!-- end col -->
	...
	@endif
    <?php $footerAboutLink = Menu::where('slug', 'footer')->get(); ?>
    <footer>
						...
						<div class="col-12 footer-widget-inner">
							<ul class="quick-link-list">
								@foreach($footerAboutLink as $linkEn)
							        <li><a href="{{ $linkEn->value }}"><i class="fa fa-chevron-circle-right"></i> {{ $linkEn['key' . ($lang == 'hi' ? '_hi' : '')] }}</a></li>
							    @endforeach
							</ul>
						</div>	
                    </div>
						...
						<div class="col-12 footer-widget-inner">
							<ul class="quick-link-list">
								@foreach($footerDepartmentLink as $Deplink)
							        <li><a href="{{ $Deplink->value }}"><i class="fa fa-chevron-circle-right"></i>{{ $Deplink['key' . ($lang == 'hi' ? '_hi' : '')] }}</a></li>
							    @endforeach
							</ul>
						</div>	
                    </div>
						...
						<div class="col-12 footer-widget-inner">
							<ul class="quick-link-list">
								@foreach($footerInformationLink as $Inforlink)
							        <li><a href="{{ $Inforlink->value }}"><i class="fa fa-chevron-circle-right"></i> {{ $Inforlink['key' . ($lang == 'hi' ? '_hi' : '')] }}</a></li>
							    @endforeach
							</ul>
						</div>	
                    </div>
						...
						<div class="col-12 footer-widget-inner">
							<ul class="quick-link-list">
								@foreach($footerOtherLinksLink as $Otherlink)
							        <li><a href="{{ $Otherlink->value }}"><i class="fa fa-chevron-circle-right"></i> {{ $Otherlink['key' . ($lang == 'hi' ? '_hi' : '')] }}</a></li>
							    @endforeach
							</ul>
						</div>	
                    </div>
                ...
                <div class="row">
                    <div class="col-lg-5 col-md-5 col-sm-12 col-12 mb-lg-0 mb-md-0 mb-4 footer-menu text-center text-lg-left text-md-left">
                        <ul>
                            ...
                            <li>
                            	<a href="{{route('screenReaderDetail',['slug' => 'disclaimer'])}}"{{ __('Disclaimer') }}</a> 
                            </li>
                            @endif
                            @if(isset($siteMapStatus->status) && $siteMapStatus->status == 1)
                            <li>
                            	<a href="{{route('screenReaderDetail',['slug' => 'site-map'])}}">{{ __('Site Map') }}</a> 
                            </li>
                            @endif
                        </ul>
                    </div>
                    <div class="col-lg-7 col-md-7 col-sm-12 col-12 copyright-text text-center text-lg-right text-md-right">
                        <p>Copyright &copy; {{ date('Y') }} <a href="javascript:;"> National Institute of Solar Energy </a> | All Rights Reserved</p>
                    </div>
                </div>
            </div>
        </div>
