...
class MenuHindiController extends AppController
{
    ...
    function index(Request $request)
    {
        ...
        if ($request->get('search')) {
            ...
            $search = '%' . $search . '%';
            $where['(menu_hindi.id LIKE ? or menu_hindi.key LIKE ? or menu_hindi.value LIKE ?)'] = [$search, $search, $search];
        }
    ...
    public function add(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            foreach ($data['menuItems'] as $menuItem)
            {
                if (isset($menuItem['id']) && !empty($menuItem['id'])) {
                    ...
                    MenuHindi::where('id', $menuItem['id'])->delete();
                    MenuHindi::create($payload);
                } else {
        ...
        return view("admin/menuHindi/add");
    }
    ...
    public function addFooterMenu(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if ($isValid) {
                ...
                // dd($data['footerItems']);
                MenuHindi::where('slug', 'LIKE', 'footer')->delete();
        ...
        if ($Menu) {
            if ($request->isMethod('post')) {
                ...
                if (!$validator->fails()) {
                    ...
                    if (MenuHindi::modify($id, $payload)) {
    ...
    {
        $menuItems = MenuHindi::select('id', 'key', 'value', 'slug', 'mega_menu as megaMenu')->get();
