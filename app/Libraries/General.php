...
class General
{
	...
	*/
	public static function hash($limit = 32)
	{
    	...
    	/**
	* To make random password hash string
	*/
	public static function password($limit = 8)
	{
		return Str::random($limit);
	}
	/**
	...
	**/
    ...
    public static function sendTemplateEmail_V2($toEmail, $templateName, $codes = [])
    {
        // Define the data to pass to the template
        $data = $codes;
        Mail::send($templateName, $data, function ($message) use ($toEmail) {
            $message->to($toEmail)
    ...
    public static function sendPasswordToUserEmail($toEmail, $templateName, $codes = [])
    {
        // Define the data to pass to the template
        $data = $codes;
        Mail::send($templateName, $data, function ($message) use ($toEmail) {
