...
class Batches extends AppModel
{
    ...
    */
    ...
    {
    	$listing = Batches::orderByRaw($orderBy);
    ...
    */
    ...
    {
    	$record = Batches::orderByRaw($orderBy);
	    foreach($where as $query => $values)
	    {
	    	if(is_array($values))
    ...
    */
    public static function create($data)
    {
        ...
        $batche->created_by = AdminAuth::getLoginId();
    	$batche->created = date('Y-m-d H:i:s');
    	$batche->modified = date('Y-m-d H:i:s');
	    if($batche->save())
	    {
            if(isset($data['title']) && $data['title'])
            {
                $batche->slug = Str::slug($batche->title) . '-' . General::encode($batche->id);
                $batche->save();
            }
	    	return $batche;
	    }
    ...
    */
    public static function modifyAll($ids, $data)
    {
    	if(!empty($ids))
    	{
    		return Batches::whereIn('batches.id', $ids)
		    		->update($data);
	    }
	    else
	    {
