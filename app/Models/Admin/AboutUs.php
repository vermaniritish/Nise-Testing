...
class AboutUs extends AppModel
{
    ...
    */
    ...
    {
    	$orderBy = $request->get('sort') ? $request->get('sort') : 'about_us.id';
    	$direction = $request->get('direction') ? $request->get('direction') : 'desc';
    	$page = $request->get('page') ? $request->get('page') : 1;
    	$limit = self::$paginationLimit;
    	$offset = ($page - 1) * $limit;
    	
    	$listing = AboutUs::select([
	    		'about_us.*',
                'owner.first_name as owner_first_name',
                'owner.last_name as owner_last_name'
	    	])
            ->leftJoin('admins as owner', 'owner.id', '=', 'about_us.created_by')
    ...
    * @param $limit
    */
    public static function getAll($select = [], $where = [], $orderBy = 'about_us.id desc', $limit = null)
    {
    	$listing = AboutUs::orderByRaw($orderBy);
    ...
    * @param $orderBy
    */
    public static function getRow($where = [], $orderBy = 'about_us.id desc')
    {
    	$record = AboutUs::orderByRaw($orderBy);
	    foreach($where as $query => $values)
	    {
	    	if(is_array($values))
    ...
    */
    public static function modifyAll($ids, $data)
    {
    	if(!empty($ids))
    	{
    		return AboutUs::whereIn('about_us.id', $ids)
		    		->update($data);
	    }
	    else
	    {
