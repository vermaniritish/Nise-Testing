...
class Slots extends AppModel
{
    ...
    */
    ...
    {
    	$orderBy = $request->get('sort') ? $request->get('sort') : 'slots.id';
    	$direction = $request->get('direction') ? $request->get('direction') : 'desc';
    	$page = $request->get('page') ? $request->get('page') : 1;
    	$limit = self::$paginationLimit;
    	$offset = ($page - 1) * $limit;

    	$listing = Slots::select([
	    		'slots.*',
                'owner.first_name as owner_first_name',
                'owner.last_name as owner_last_name'
	    	])
            ->leftJoin('admins as owner', 'owner.id', '=', 'slots.created_by')
    ...
    * @param $limit
    */
    public static function getAll($select = [], $where = [], $orderBy = 'slots.id desc', $limit = null)
    {
    	$listing = Slots::orderByRaw($orderBy);
    ...
    * @param $orderBy
    */
    public static function getRow($where = [], $orderBy = 'slots.id desc')
    {
    	$record = Slots::orderByRaw($orderBy);
	    foreach($where as $query => $values)
	    {
	    	if(is_array($values))
    ...
    */
    public static function modifyAll($ids, $data)
    {
    	if(!empty($ids))
    	{
    		return Slots::whereIn('slots.id', $ids)
		    		->update($data);
	    }
	    else
	    {
