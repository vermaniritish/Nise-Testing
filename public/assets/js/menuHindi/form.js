var menu = new Vue({
    ...
    methods: {
        async fetchMenuItems() {
            try {
                const response = await fetch(admin_url + '/hi/menu/getMenuItems', {
                    method: 'GET',
                    headers: {
        ...
        async removeItem(index) {
            ...
            if (item.id) {
                if (confirm('Are you sure you want to delete this item?')) {
                    ...
                    try {
                        let response = await fetch(admin_url + `/hi/menu/delete/${item.id}`, {
                            method: 'DELETE',
                            headers: {
        ...
        async submitForm()
        {
            ...
            try {
                    ...
                    response = await fetch(admin_url + '/hi/header/menu/add', {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': csrfToken,
                            'Accept': 'application/json',
        ...
        async submitMegaForm(k) {
            ...
            const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
            try {
                let response = await fetch(admin_url + '/hi/header/menu/add', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': csrfToken,
                        'Accept': 'application/json',
        ...
        async submitFooterForm() {
            ...
            const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
            try {
                let response = await fetch(admin_url + '/hi/footer-menu/add', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': csrfToken,
                        'Accept': 'application/json',
